...
## 2026-07-13 — Re-migrated MCQs to a NEW Supabase project (project ref changed!)

### Important: project ref changed
The Supabase project was recreated — old ref `lcsuasddoertvoujwsgo` is gone.
Current linked project: **`lqopullrswpgqccklnie`** (org `taleemkasafar`,
region ap-southeast-2, created 2026-07-12). `.env.local` and
`supabase/.temp/project-ref` already point to the new ref. All 14 `mcq_*`
migrations were already present on the new project (schema intact), but all
content tables were empty (fresh data-wise) — only `profiles` had 1 row.

### MCP reconfigured
Supabase MCP is now provided via the **supabase-hosted Kiro Power** (browser
OAuth, no manual mcp.json edit needed/possible — `.kiro/settings/mcp.json` is
protected). Use `kiro_powers` tool with `powerName: "supabase-hosted"`,
`serverName: "supabase"`.

### Data re-imported
Ran `mcqs/import_batches/batch_00_setup.sql` (entry_test + subjects +
test_subjects + 41 topics) via MCP `execute_sql`, then
`batch_01_questions.sql` .. `batch_10_questions.sql` via
`supabase db query --linked --file <path>` (CLI, not MCP — batches are
~300KB each, too large to relay through MCP one-shot; CLI hits the Management
API directly). All batches are idempotent (`on conflict do update`).

Verified counts match the original 2026-06-25 import exactly:
- 1169 questions, 4676 options, 1169 question_tests, 51 flagged.
- Per subject: english 315 (4 topics), maths 357 (16 chapters), physics 497
  (21 chapters).
- Advisors clean: only expected WARNs (SECURITY DEFINER grading RPCs +
  is_admin/rls_auto_enable, leaked-password-protection still off) and INFO
  unused-index notices (no traffic yet). No new issues introduced.

### Next
- Regenerate `lib/database.types.ts` if not already matching this project
  (schema didn't change, so likely still valid — verify on next schema touch).
- Continue with product work; data layer is ready.
